          ...
          <TabsContent value="delivery" className="m-0 flex-1 min-h-0">
            <Tabs defaultValue="not-delivered" className="flex flex-col h-full min-h-0">
              ...
              <TabsContent value="not-received" className="m-0 flex-1 min-h-0" style={{ overflow: 'auto', maxHeight: '45vh' }}>
                <CustomerList customers={deliveryNotReceived} emptyMessage="لا توجد زيارات بدون تسليم" onCustomerClick={(c) => handleCustomerClick(c, 'delivery')} showActionButtons onClosed={handleCustomerClosed} onUnavailable={handleCustomerUnavailable} checkingLocationFor={checkinsectors={sectors} gLocationFor} loadsectors={sectors} ingFor={loadingDelsectors={sectors} iveryFor} searchQusectors={sectors} ery={searchQuery} />
              </TabsContent>
              <TabsContent value="received" className="m-0 flex-1 min-h-0" style={{ overflow: 'auto', maxHeight: '45vh' }}>
                <CustomerList customers={deliveryReceived} emptyMessage="لا توجد توصيلات بعد" onCustomerClick={handleShowDeliveredOrderDetails} showPrintButton onPrint={handlePrintDeliveresectors={sectors} dOrder} checkingLocationFor={checkinsectors={sectors} gLocationFor} loadingFor={loadingDelsectors={sectors} iveryFor} searchQuery={searchQuery} />
              </TabsContent>
            </Tabs>
          </TabsContent>
              ...
              <TabsContent value="visited-no-order" className="m-0 flex-1 min-h-0">
                <Tabs defaultValue="visit-only" className="flex flex-col h-full min-h-0">
                  ...
                  <TabsContent value="visit-only" className="m-0 flex-1 min-h-0" style={{ overflow: 'auto', maxHeight: '40vh' }}>
                    <CustomerList customers={salesVisitedOnly} emptyMessage="لا توجد زيارات بدون طلبيات" onCustomerClick={(c) => handleCustomerClick(c, 'salessectors={sectors} ')} checkingLocationFor={checkingLocationFor} searchQuery={searchQuery} />
                  </TabsContent>
